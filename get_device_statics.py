...
def update_info():
    global msgType, open_time, open_seconds, present_type, type_ct_list, uFull, faultreport_time, faultType, restarttime, uptime, weights, volumes, oneday_time, offline_flag, info_cache
    ...
    while(True):
        ...
        if update_info() == 0:
           ...
           print("检测到计算机已离线，请连接到网络后再试")
            ...
            #写入data.xlsx
            writer = pd.ExcelWriter('data.xlsx')
            data.to_excel(writer, sheet_name = 'Sheet1', index = False, header = ['日期', '最后一次开盖时间', '日开盖次数', '开盖持续时间', '当前垃圾类型', '干垃圾投放次数', '湿垃圾投放次数', '可回收垃圾投放次数', '有害垃圾投放次数', '桶满状态', '故障类型', '故障上报时间', '重启上报时间', '换桶时间', '垃圾重量', '垃圾体积', '每日上报时间'])
            writer.save()
